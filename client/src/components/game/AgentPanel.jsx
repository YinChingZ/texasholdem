...
export default function AgentPanel({ gameState, onCommand }) {
  ...
  const pairing = pending && secret?.grantExpiresAt === agent.expiresAt && secret?.code ? secret : null
  const copy = async (text, success) => { try { await navigator.clipboard.writeText(text); setMessage(success) } catch { setMessage('复制失败，请手动选择并复制') } }
  ...
  const command = async event => {
    ...
    try {
      ...
      else if (result.pairingCode) setSecret({ code: result.pairingCode, expiresAt: result.pairingExpiresAt, grantExpiresAt: result.snapshot.self.agent.expiresAt })
      else if (result.agentToken) setSecret({ token: result.agentToken, expiresAt: result.snapshot.self.agent.expiresAt })
      else if (['createAgentGrant', 'createAgentPairing'].includes(event)) setMessage('授权已生成，但凭证未收到。请重新生成。')
  ...
  return <section className={styles.bar} aria-label="Agent 托管">
    ...
    <MobileSheet open={open} title="Agent 托管" onClose={close}>
      <div className={styles.panel}>
        <AgentSetup apiUrl={apiUrl} pairing={pairing} pending={pending} controlled={controlled} busy={busy} ended={gameState.phase === 'ENDED'} onGenerate={() => command('createAgentPairing')} onCopy={copy} />
        ...
        <details><summary>高级：本地 MCP / HTTP 凭证</summary>
